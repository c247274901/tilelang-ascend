...
死锁或性能异常前先确认 `auto_sync` 已关（手写 flag 时）。`auto_sync=True` 会让 pass 重复插入核内 flag，与手写 flag 冲突导致死锁或数据错乱。手写 `cross_flag` 时 `auto_cv_sync`/`auto_cv_combine` 也须关。完整 pass_configs 配置见 `tilelang-mode-guide` skill。
...
3. **set/wait 属于三类合法模式吗？** —— 模式 A（严格配对）/ B（一次性通知）/ C（混合节流），定义和示例见 [flag-mechanism.md §3](flag-mechanism.md)。
...
模式 C 的 cross_flag 会按 cross_interval 节流，循环内 set 和 wait 都应按批次数出现。init set 只用于第一轮资源信用，不表示一个 set 可被多个 wait 消费；统计 count 时要确认 init、set 节流条件和 wait 节流条件都对齐。
...
5. **尾部等待位置错**：在 V 还没读完 workspace 时 C 就退出或覆盖数据，或反之。cross_flag 不需要核内 flag 那种对称 clear，但仍可能需要尾部 wait 来保证 workspace 生命周期。
