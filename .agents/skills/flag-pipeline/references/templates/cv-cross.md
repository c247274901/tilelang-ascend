...
CV 融合算子（FlashAttention、matmul+activation、GEMM+softmax+GEMM）的标准模板，对齐 `examples/flash_attention/fa_opt/flash_attn_bhsd_expert_h16_d128.py` 的 expert 范本。
...
循环内 set 和 wait 都按 cross_interval 节流，数量应按批次数对齐。init set 只提供第一轮资源可用信用，不代表一个 set 可以被多个 wait 消费。关键确认点：init 借了信用、set/wait 节流条件对齐、节流条件含 `or i==N-1`、workspace num_stages ≥ cross_interval。
...
- init 借信用的方向要理解清楚：`SEM_WS1_V2C` 是 V→C 方向，意思是"V告诉C：ws1 你可以写"。kernel 启动时 V 先 set 这个，C 第一轮 wait V2C 就不会阻塞。init set 的 pipe 不表示数据写回完成，只是预借资源信用；按模板使用 `"MTE2"`，不要把它和数据 ready 的 `"MTE3"` 混淆。
...
cross_flag 的尾部等待和核内 flag 的 clear 不同，不需要 `wait_cross_flag` 回所有 init 借的信用，而是：
...
判断是否需要额外尾部 wait：看 kernel 结束时是否必须确认**对方已经完成最后一批 workspace 读/写**。如果循环结构保证最后一轮 set 被对侧 wait 消费，且没有 workspace 生命周期风险，就不需要额外 wait。

### 7.3 信用流转验证

写完全部 flag 后，对每个 SEM id 按以下步骤验证：
1. init 有没有 set？第一轮 wait 会不会阻塞？
2. 循环内 set 和 wait 的条件对齐吗？（C set 条件 `(i+1)%cross_interval==0 or i==N-1` 对应 V wait 条件 `i%cross_interval==0`）
