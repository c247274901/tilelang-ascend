...
- **解法**：对照三类合法模式验证（A 严格配对 / B 纯广播 / C 混合节流）。
...
### 1.8 pass_configs 没全关

- **现象**：手写 flag 的 kernel 死锁或性能回退。
- **根因**：pass（AUTO_SYNC 等）又插了一遍同步，和手写的 flag 冲突或重复。
- **解法**：手写 flag 时四件套全 False。见 §4。
...
- **现象**：编译错误。
- **根因**：`S1 × block_M × K_L1 × sizeof(dtype) > 512KB`。
- **解法**：减小 S1 或 K_L1。
...
对性能有要求、使用 expert 模式手写 flag 时，四件套全 False。每个开关关掉的 pass：

| 开关 | 关掉的 pass | 不关的后果 |
|---|---|---|
| `TL_ASCEND_AUTO_SYNC` | AscendSyncInsert（核内自动 set/wait_flag） | 手写 flag + pass 插的 flag 重复，死锁或回退 |
| `TL_ASCEND_AUTO_CV_SYNC` | 核间自动 cross_flag 插入 | 手写 cross_flag + pass 插的 cross_flag 重复 |
| `TL_ASCEND_AUTO_CV_COMBINE` | CrossCorePipeline（自动拆 CV stage） | pass 把 CV 循环重写成多 stage，破坏手写结构 |
| `TL_ASCEND_MEMORY_PLANNING` | AscendMemoryPlanning（自动 buffer 规划） | pass 重排 buffer 偏移，可能和手写的 workspace 维度冲突 |

手写 flag = expert 模式 = 四件套全 False。不能 C 手写 + V 自动混合——AUTO_SYNC 是函数级开关，要么给整个 kernel 插 flag，要么不插，无法只让 V 侧插、C 侧不插。C 侧手写 flag + 全局 AUTO_SYNC=True 时，pass 会在 C 侧也插一遍，和手写 flag 冲突。

## 4. 性能回退类

### 4.1 barrier_all 没换成 flag
...
| pass_configs 没全关 | 死锁/回退 | pass 重复插同步 | 四件套全 False |
