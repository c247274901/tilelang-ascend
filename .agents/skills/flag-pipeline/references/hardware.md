...
S1（L1 深度）和 S2（L0 深度）的上限由硬件容量和分块大小共同决定。L1 是 A/B 共享的，容量必须按合计占用检查，不能让 A_L1 和 B_L1 分别各占 512KB。
...
S1 × block_M × K_L1   × sizeof(dtype)           (A_L1)
+ S1 × K_L1 × block_N × sizeof(dtype)           (B_L1)
+ extra_L1_buffers                              (bias/scale/temp, if any)
≤ 512KB                                         (L1 total)
...
示例 `block_M=128, block_N=256, K_L1=512, S1=2, fp16`：

```
A_L1 = 2 × 128 × 512 × 2B = 256KB
B_L1 = 2 × 512 × 256 × 2B = 512KB
A_L1 + B_L1 = 768KB ❌ 超过 L1 512KB
```

所以即使 A/B 分量单独看不超，也必须按总和判断。
