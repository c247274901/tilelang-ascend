...
- L1: S1=2~4，`S1 × (block_M + block_N) × K_L1 × sizeof(dtype) + extra_L1_buffers ≤ 512KB`
- L0C: 不轮转但大小受限，`block_M × block_N × sizeof(accum) ≤ 128KB`
- UB: S_vec=2，`S_vec × rows × cols × sizeof(dtype) ≤ 192KB`

buffer shape 第一维永远是 stage 轮转维，使用 `T.alloc_L1/alloc_L0A/alloc_L0B/alloc_L0C/alloc_ub` 等分层 buffer API，并用 `% stages` 实现 ping-pong。
...
- **模式 B（一次性通知）**：set 1 次，只能 wait 1 次（少用）
- **模式 C（混合节流）**：init 借信用 + 生产者/消费者按同一节流粒度 set/wait（FA 模式）

核心原则：**init 借信用，clear 归还；谁等待谁 wait，谁完成谁 set**。

### Step 4: 配置 pass_configs

pass_configs 的完整配置指南见 `tilelang-mode-guide` skill。手写 flag 的核心约束：`auto_sync` 必须关，否则 pass 会重复插入核内 `set/wait_flag`，与手写 flag 冲突导致死锁或数据错乱；手写 `cross_flag` 时 `auto_cv_sync`/`auto_cv_combine` 也须关。其余开关按场景选。
...
2. **pipe 参数必须匹配**：数据生产完成后的 cross_flag，Cube 写 GM 后用 `"FIX"`，Vector 写 GM 后用 `"MTE3"`。init 借信用不代表数据 ready，按模板使用指定 pipe。
